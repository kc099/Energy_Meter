#!/usr/bin/env python3
"""Andon station client posting encrypted telemetry to the Django API."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Provision an Andon device and push encrypted telemetry to the Django API.",
    )
    parser.add_argument("--host", default=os.getenv("ANDON_HOST", "localhost"), help="API host without protocol")
    parser.add_argument(
        "--port",
        type=int,
        default=int(os.getenv("ANDON_PORT", 8000)),
    ...
    parser.add_argument(
        "--state-file",
        type=Path,
        default=Path(os.getenv("ANDON_STATE_FILE", "andon_client_state.json")),
        help="Where API keys and AES keys are cached",
    )
    parser.add_argument(
        "--telemetry-interval",
        type=float,
        default=float(os.getenv("ANDON_TELEMETRY_INTERVAL", 5.0)),
    ...
    parser.add_argument(
        "--station-id",
        type=int,
        default=_env_int("ANDON_STATION_ID"),
        help="Fallback Andon station ID if state file lacks one",
    )
    parser.add_argument("--once", action="store_true", help="Send a single telemetry frame instead of looping")
    parser.add_argument(
        "--min-level",
        type=float,
        default=float(os.getenv("ANDON_MIN_ACTUAL", 0.0)),
        help="Lower bound for synthetic actual count generation",
    )
    parser.add_argument(
        "--max-level",
        type=float,
        default=float(os.getenv("ANDON_MAX_ACTUAL", 100.0)),
        help="Upper bound for synthetic actual count generation",
    )
    parser.add_argument(
        "--unit",
        default=os.getenv("ANDON_UNIT", "units"),
        help="Measurement unit reported with telemetry",
    )
    parser.add_argument(
        "--timeout",
        type=float,
        default=float(os.getenv("ANDON_TIMEOUT", 15.0)),
...
def load_state(path: Path) -> Dict[str, str]:
    ...
    state.setdefault("station_id", None)
...
def claim_token(session: requests.Session, cfg: ClientConfig, state: Dict[str, str]) -> None:
    ...
    station_id = state.get("station_id") or cfg.station_id
    payload = {"token": token}
    if station_id is not None:
        payload["station_id"] = station_id
    response = session.post(url, json=payload, timeout=cfg.timeout)
    ...
    state["telemetry_url"] = ingest_url if "andon" in ingest_url else ""
...
def build_telemetry_payload(cfg: ClientConfig) -> Dict[str, float | str]:
    actual = random.uniform(cfg.min_level, cfg.max_level)
    downtime = random.uniform(0, 10)
    plan = max(actual, cfg.max_level * 0.9)
    return {
...
def _resolve_station_id(cfg: ClientConfig, state: Dict[str, str]) -> int:
    candidate = state.get("station_id")
    if candidate in (None, "") and cfg.station_id is not None:
        candidate = cfg.station_id
        state["station_id"] = candidate
        save_state(cfg.state_file, state)

    if candidate in (None, ""):
        raise RuntimeError(
            "Station identifier missing. Re-run provisioning or pass --station-id."
        )

    try:
        station_id = int(candidate)
    except (TypeError, ValueError) as exc:
        raise RuntimeError("Stored station identifier is not a valid integer") from exc
...
def _resolve_telemetry_url(cfg: ClientConfig, state: Dict[str, str], station_id: int) -> str:
    telemetry_url = state.get("telemetry_url") or ""
    if telemetry_url:
        return telemetry_url

    telemetry_url = build_base_url(cfg) + f"/andon/api/stations/{station_id}/telemetry/"
...
def send_telemetry(session: requests.Session, cfg: ClientConfig, state: Dict[str, str]) -> None:
    ...
    station_id = _resolve_station_id(cfg, state)
    url = _resolve_telemetry_url(cfg, state, station_id)
...
def main() -> int:
    ...
    if args.provisioning_token:
        token_value = args.provisioning_token.strip()
        state.update(
            {
                "provisioning_token": token_value,
                "api_key": "",
                "aes_key_b64": "",
                "telemetry_url": "",
                "station_id": None,
    ...
    try:
        while True:
            try:
                if state.get("provisioning_token"):
                    claim_token(session, cfg, state)
                if not state.get("api_key"):
                    claim_token(session, cfg, state)
                    if not state.get("api_key"):
                        raise RuntimeError("Provisioning did not return an API key")
